...
The biggest challenge is to convert string to Lisp list.  After a quick search with =describe-function=, =read= function is able to perfectly handle that in this case.
...
#+begin_src emacs-lisp :dir /tmp :results output :exports both :eval never-export
